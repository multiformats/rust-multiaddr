...
fn assert_bytes_all(source: &str, target: &[u8], protocols: Vec<Protocols>) -> () {
...
#[test]
fn byte_formats() {
    assert_bytes_all("/ip4/1.2.3.4", &[0, 4, 1, 2, 3, 4], vec![Protocols::IP4]);
    assert_bytes_all("/ip4/0.0.0.0", &[0, 4, 0, 0, 0, 0], vec![Protocols::IP4]);
    ...
    // assert_bytes_all("/onion/timaq4ygg2iegci7:1234", &[], vec![Protocols::Onion]);
    // assert_bytes_all("/onion/timaq4ygg2iegci7:80/http", &[], vec![Protocols::Onion]);
    assert_bytes_all("/udp/0", &[0, 17, 0, 0], vec![Protocols::UDP]);
    assert_bytes_all("/tcp/0", &[0, 6, 0, 0], vec![Protocols::TCP]);
    assert_bytes_all("/sctp/0", &[0, 132, 0, 0], vec![Protocols::SCTP]);
    assert_bytes_all("/udp/1234", &[0, 17, 4, 210], vec![Protocols::UDP]);
    assert_bytes_all("/tcp/1234", &[0, 6, 4, 210], vec![Protocols::TCP]);
    assert_bytes_all("/sctp/1234", &[0, 132, 4, 210], vec![Protocols::SCTP]);
    assert_bytes_all("/udp/65535", &[0, 17, 255, 255], vec![Protocols::UDP]);
    assert_bytes_all("/tcp/65535", &[0, 6, 255, 255], vec![Protocols::TCP]);
    // assert_bytes_all("/ipfs/QmcgpsyWgH8Y8ajJz1Cu72KnS5uo2Aa2LpzU7kinSupNKC", &[], vec![Protocols::IPFS]);
